...
public class QDiscussDialog extends JDialog {
...
//        try {
//            QDiscussDialog dialog = new QDiscussDialog("修改信息","科目1", "60");
...
//    }
     ...
     * Create the dialog
     */
    public QDiscussDialog(String title, String kemu, String Qno, Connection c) {
        super();
        con=new Dao(c);
        setModal(true);
        ...
        submitButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (Qno.equals("")){
                    ...
                    String sql = "INSERT INTO Discussion VALUES('" + '6' + text_Qno1.getText() + "','" + text_question.getText() + "','";
                    ...
                    try{
                        if (text_answer.getText().trim().length()==0 || text_question.getText().trim().length() == 0) {
                            ...
                            text_answer.setText("");
                            JOptionPane.showMessageDialog(null, "插入论述题成功！", "插入", JOptionPane.INFORMATION_MESSAGE);
                        }
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(null, "插入论述题失败，请检查数据！", "插入", JOptionPane.OK_OPTION);
                    }
                }
                else if (Qno!=""){
                    //先删除
                    String sql1 = "delete from Discussion where Qno='"+Qno+"';";
                    try{
                        con.dataUpdate(sql1);
                    ...
                    String point = combo_point.getSelectedItem().toString();
                    int diff = combo_diff.getSelectedIndex() + 1;
                    String sql = "INSERT INTO Discussion VALUES('" + text_QTno1.getText() + text_Qno1.getText() + "','" + text_question.getText() + "','";
                    try{
                        if (text_answer.getText().trim().length()==0 || text_question.getText().trim().length() == 0) {
                            JOptionPane.showMessageDialog(null, "插入数据不完整！", "插入", JOptionPane.OK_OPTION);
                        } else {
                            sql += text_answer.getText()+ "','" + point + "'," + diff + ",0,'"+kemu+"');";
                            con.dataUpdate(sql);
                            JOptionPane.showMessageDialog(null, "修改成功！", "插入", JOptionPane.INFORMATION_MESSAGE);
                        }
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(null, "修改数据失败，请检查数据！", "插入", JOptionPane.OK_OPTION);
                    }
                }
            }
        });
        ...
        if (Qno!= "") {
            String sql = "select * from Discussion where Qno='" + Qno + "';";
            System.out.println(sql);
            ResultSet rs1 = con.getRs(sql);
            try {
                while (rs1.next()) {
                    String oldQno = rs1.getString("Qno");
                    text_Qno1.setText(oldQno.replaceFirst("6",""));
                ...
                Logger.getLogger(QDiscussDialog.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
