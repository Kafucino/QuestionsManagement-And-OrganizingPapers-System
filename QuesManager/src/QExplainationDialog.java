...
public class QExplainationDialog extends JDialog {
...
//        try {
//            QExplainationDialog dialog = new QExplainationDialog("修改信息","科目1", "40");
...
//    }
    ...
    public  QExplainationDialog(String title, String kemu, String Qno, Connection c ) {
                ...
                if (Qno.equals("")){
                    ...
                    try{
                        if (text_answer.getText().trim().length()==0 || text_question.getText().trim().length() == 0) {
                            ...
                            text_answer.setText("");
                            JOptionPane.showMessageDialog(null, "插入名词解释题成功！", "插入", JOptionPane.INFORMATION_MESSAGE);
                        }
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(null, "插入名词解释失败，请检查数据！", "插入", JOptionPane.OK_OPTION);
                    }
                }
                else if (Qno!=""){
                    //先删除
                    String sql1 = "delete from explanation where Qno='"+Qno+"';";
                    try{
                        con.dataUpdate(sql1);
                    ...
                    try{
                        ...
                        JOptionPane.showMessageDialog(null, "插入名词解释题失败，请检查数据！", "插入", JOptionPane.OK_OPTION);
                    }
                }
            }
        });
        ...
        if (Qno!= "") {
            String sql = "select * from explanation where Qno='" + Qno + "';";
            System.out.println(sql);
            ResultSet rs1 = con.getRs(sql);
            try {
                while (rs1.next()) {
                    String oldQno = rs1.getString("Qno");
                    text_Qno1.setText(oldQno.replaceFirst("4",""));
                ...
                Logger.getLogger(QExplainationDialog.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
